...
model.asli <- glm(class ~ x, data=data.tr, 
                  family="binomial")
model.asli

#memprediksi data testing dan menghitung akurasinya
prediksi.prob.asli <- predict(model.asli, 
                              newdata=data.ts, 
                              type="response")
prediksi.asli <- ifelse(prediksi.prob.asli > 0.5, 
                        1, 0)
...
library(classInt)
eqwid <- classIntervals(data.tr$x, 10, 
                        style = 'equal')
data.tr$x.disk <- cut(data.tr$x, 
                      breaks=eqwid$brks, 
                      include.lowest=TRUE)

#membuat model regresi logistik dengan x hasil diskretisasi
model.disk <- glm(class ~ x.disk, data=data.tr, 
                  family="binomial")

#memprediksi data testing dan menghitung akurasinya
data.ts$x.disk <- cut(data.ts$x, breaks=eqwid$brks, include.lowest=TRUE)
prediksi.prob.disk <- predict(model.disk, 
                              newdata=data.ts, 
                              type="response")
